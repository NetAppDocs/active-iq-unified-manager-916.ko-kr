...
summary: Active IQ Unified Manager REST API는 HTTP 요청을 발행할 수 있는 모든 웹 브라우저나 프로그래밍 플랫폼을 사용하여 접근할 수 있습니다.  Unified Manager는 기본 HTTP 인증 메커니즘을 지원합니다.  Unified Manager REST API를 호출하기 전에 사용자를 인증해야 합니다. 
---
...
----
+
** `_IP address/hostname_`API 서버의 IP 주소 또는 정규화된 도메인 이름(FQDN)입니다.
** 포트 443
+
