...
summary: Active IQ Unified Manager 다른 API를 실행하는 동안 수행된 작업에 대한 정보를 검색하는 작업 API를 제공합니다. Job 객체를 사용하여 비동기 처리가 어떻게 작동하는지 알아야 합니다. 
---
= 작업 객체 비동기 프로세스
:allow-uri-read: 
:icons: font
...
. 루프 내에서 Job 객체가 최종 상태에 도달할 때까지 기다립니다. `NORMAL, ERROR,` 또는 `PARTIAL_FAILURES.`
. 작업의 최종 상태를 확인하고 작업 결과를 검색합니다.
